...
<head>
    ...
    <title>RecipeFlo - Modifier Recette</title>
...
<body class="bg-gray-50">
    ...
    <main class="container mx-auto px-6 py-8">
        @php
            $action = route('recettes.update', $recipe->id);
            $method = 'PUT';
            $submitLabel = 'Enregistrer les modifications';
        @endphp
        @include('recipes.form', compact('categories', 'ingredients', 'recipe', 'action', 'method', 'submitLabel'))
        @auth
        <form action="{{ route('recettes.destroy', $recipe->id) }}" method="POST" onsubmit="return confirm('Supprimer cette recette ?');" class="inline-block ml-2">
            @csrf
            @method('DELETE')
        ...
        @endauth
        <a href="{{ route('categories.index') }}" class="ml-2 text-blue-500 underline">Gérer les catégories</a>
    </main>
    <!-- Footer -->
        ...
        document.getElementById('recipe-form').addEventListener('submit', async function(e) {
            ...
            try {
                const response = await fetch('/api/recipes/{{ $recipe->id }}', {
                    method: 'POST', // fetch ne supporte pas PUT avec FormData, donc on utilise POST + _method
                    headers: {
                ...
                if (response.ok) {
                    window.location.href = '/recettes/{{ $recipe->id }}/edit';
                } else {
